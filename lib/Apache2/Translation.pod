...
a particular interface, see below. Currently there is one provider
implemented, L<Apache2::Translation::DB>. It reads the configuration
from a database via L<DBI>.
...
examples. Other directives insist on being placed in a container, some
even want to be only in a C<Directory> container. For example C<AllowOverride>
demands a C<Directory> container.
...
affects the way Apache maps the request file name computed in the translation
phase to the file system, because that comes later. But it also means, your
...
This is also the only form of this action, that is available with mod_perl
before 2.0.3.
...
This last form evaluates configuration directives as if they appear outside
any container. It is also a little dangerous, because it causes segfaults
for some directives that are not prepared to be used this way. But it
is the only way to apply an C<AllowOverride>, for example.
...
=item B<CgiScript> (without parameter)
...
=item B<PerlScript> (without parameter)
...
This sets the initial value for the key. Default is the empty string.
